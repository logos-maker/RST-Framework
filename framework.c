...
plugPtr plugInstructionDecoder(plugHeader *plugin, int32_t opCode, int32_t index, plugPtr value, void *ptr, float opt){ // Pointer to this function is used in the myplugin header
	...
	switch(opCode){
		case plugSupports:
		    ...
		    if (0 == strcmp(((char*)ptr+10),"Events"))		return 1;  //
		...
		case plugGetParamName:          getParameterName(index, (char*)ptr);            return 1;	// Host whant the plug to transfer the indexed parameter's name. 
		...
		case plugClose:			free(plug);							return 1;	 // This op-code is sent by host before the plug gets deallocated from the system. To free resources and so on.
