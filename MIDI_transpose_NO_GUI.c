...
char product_name[] = "DSC - MIDI TRANSPOSE";	// Place your plug name inside ""
...
void getParameterName(int32_t index,  char* ptr){
        switch(index){ // Copy the name of the the right paramter to be displayed in the host.
                case  0: strcpy(ptr, "TRANSPOSE");	break; // Name of the first  parameter is between ""
...
int32_t MIDI_in(plug_instance* plug,struct plugEvents* ev){		// Take care of incomming MIDI events/messages
	...
	struct plugEvents midiout;
	midiout.number_of_events = 1;
	plugMidiEvent Event[2];
	midiout.MIDIMessages[0] = (plugMidiEvent*)Event;
	midiout.MIDIMessages[0]->eventType = 1;
	midiout.MIDIMessages[0]->eventSize = 32;
	...
	for (int32_t i = 0; i < ev->number_of_events; i++) {		// Parse/loop through all incomming events.
		...
		switch (midiData[0] & 0xf0) { // Recive on all channels
			case 0x90:  // Note on
				Event->MIDIByte[0] = 0x90; 	     					// note-on
				Event->MIDIByte[1] = midiData[1] + (char)(plug->pth.knob[0] * 64) - 32; // Note
				Event->MIDIByte[2] = midiData[2] ; 					// Velocity
				plug->hostcall(&plug->plughead, dawProcessMIDI,   0, 0, (void*)&midiout, 0); // send new knob value to the DAW.
			break;      
			case 0x80:  // Note off
				midiout.MIDIMessages[0]->MIDIByte[0] = 0x80;	     // note-off
				midiout.MIDIMessages[0]->MIDIByte[1] = midiData[1] + (char)(plug->pth.knob[0] * 64) - 32; // Note
				midiout.MIDIMessages[0]->MIDIByte[2] = midiData[2] ; // Velocity
				plug->hostcall(&plug->plughead, dawProcessMIDI,   0, 0, (void*)&midiout, 0); // send new knob value to the DAW.
			break;
